...
**P4.3 · Retrieval iteration to floor** `[x]` *(spends: cents)*
...
**P4.4 · Threshold calibration** `[x]` *(spends: cents)*
...
**P4.5 · Judge, noise, baseline** `[x]` *(spends: ~$1 total)*
...
ci-fix · 2026-07-23 · CI was RED since P3.3 landed /api/ask (I verified local builds green but only confirmed PUSHED CI green at P3.2, pre-route — a real process miss, owned). Two causes fixed: (a) `next build` collects page data for /api/ask, which imports boot-parsed env.ts, and CI provided no env vars -> gave the Build step schema-valid creds-free placeholders (build calls no APIs; real values on Vercel at deploy); this was a7e3719's Build failure. (b) machine-generated eval JSON artifacts aren't prettier-formatted -> added to .prettierignore (hand-authored testset.json stays checked); this landed with P4.2. Pushed 661086d; PUSHED CI VERIFIED GREEN: https://github.com/BrandonChurch93/Ask-the-Claude-Docs/actions/runs/30048049194 — so P3.3->P4.4 are now CI-confirmed. CONVENTION AMENDMENT (Brandon-authorized): every build-log line for a pushed step from here on records its CI run URL as evidence, not memory.
P4.4 · 2026-07-23 · Threshold calibration + the decline-sentinel hardening. CALIBRATION (eval-harness §7): top-1 similarity recorded for all 28 test questions + 5 clearly-off-corpus probes; committed distribution artifact evals/calibration.json (EVAL-15) + `npm run eval:calibrate`. FINDING (Brandon's words): the server gate's honest job is clearly-off-corpus. THREE bands, not two: clearly-off-corpus ~0.20 (pizza/France/taxes 0.20-0.21); an OVERLAP zone 0.45-0.64 where the plausible refusals (0.569-0.643) interleave with weak-but-real answerables (a-04 0.454, a-18 0.522, a-09 0.562, ...); strong answerables 0.65-0.81. NO clean gap between refusal and answerable: cosine cannot separate "adjacent but unanswered" from "answered", because a plausible refusal (EVAL-01, not gibberish) retrieves adjacent chunks at ~0.6. A clean gap DOES exist between clearly-off-corpus (0.211 max) and real content (0.454 min); T=0.35 sits there. Three commitments were mutually incompatible (deterministic server-refusal EVAL-09 needs refusals < T; plausible refusals EVAL-01 score ~0.6; answerable recall AC-01 needs weak answerables > T) -> pick two. RESOLUTION (Brandon, Option 1 + sentinel hardening): T=0.35 protects recall and catches clearly-off-corpus; plausible refusals pass the gate and decline MODEL-SIDE via a decline sentinel. FIVE RULE-1 AMENDMENTS: (1) T=0.35 CALIBRATED in config (calibratedAt 2026-07-23 + calibrationRunId), UNCALIBRATED marker gone. (2) rag-design §7: the system prompt requires the response begin with the exact sentinel "The Claude Code documentation doesn't cover this." (lib/rag/prompt.ts DECLINE_SENTINEL, one shared string for prompt + eval + UI, matches the existing refusal copy). (3) eval-harness §3/EVAL-09 + success-criteria AC-04: two-tier refusal scoring, pass = server-refused (metadata) OR response begins with the sentinel (deterministic prefix), judged no-fabrication is the backstop for a sentinel miss. (4) ui-ux-spec §7: two decline species, server refusal (full refusal state) vs model-side decline (sentinel-detected, honest generation receipt; P5 renders it). (5) Option 3 (smarter gate: margin/relevance signal) logged as a designated post-v1 improvement, deliberately not v1. VERIFICATION (Brandon req, ~1 cent live): sentinel compliance 8/8 -> all 5 plausible refusals + both refuse-boundary (b-01 TTS, b-02 max-file-size) declined with the exact sentinel prefix; the answer-boundary b-03 (scheduled PR review) answered. Eval still 0.8500/0.6583 (metrics read raw top-k, independent of T). Self-audit EVAL-14/15 + RAG-15 green, UNCALIBRATED gone; 83 tests (2 new: sentinel, config-calibrated); tsc/lint/format/DS-14/SEC-01/build clean.
docs · 2026-07-23 · P4.4 refusal amendments (Brandon-authorized, rule 1): rag-design §7 (decline sentinel in the system prompt), eval-harness §3/EVAL-09 + success-criteria AC-04 (two-tier deterministic refusal scoring: server metadata OR sentinel prefix), ui-ux-spec §7 (two decline species: server refusal vs sentinel-detected model-side decline with an honest receipt). Rationale: calibration proved plausible off-corpus questions are not cosine-separable from weak answerables, so the model-side decline carries the refusal load for them and the sentinel unifies detection + rendering. Committed separately as `docs: P4.4 refusal amendments (decline sentinel + two-tier scoring)`.
P4.3 · 2026-07-23 · Retrieval iteration to floor. LADDER: baseline 0.6000/0.4708 -> (1) five same-page gold-label corrections (a-06/07/08/12/20; parent/sibling chunks retrieval already returned in top-5, each argued in the testset changelog, EVAL-03) -> 0.8500/0.6583, BOTH FLOORS MET (AC-01 hit@5 >=0.85, AC-02 MRR >=0.60). (2) changelog + whats-new excluded from the corpus: a product fix (killed changelog pollution across all queries) that did NOT lift the eval (a-18 stayed a miss). FINAL: hit@5 0.8500, MRR 0.6583; 3 honest residuals a-10/a-11/a-18 (genuine retrieval gaps recorded in the testset changelog: the canonical gold page never reaches top-5; no sanctioned fix; crossing pages to whatever retrieval returned would be inflation, declined). Zero-margin at 0.85 explicitly accepted: a future flip below floor is CI doing its job, not a number to pre-inflate against. a-08's gold flagged the weakest correction (general-MCP page intro) to be re-argued not defended if P4.4 distributions or judged evals cast doubt.
CHANGELOG-EXCLUSION ARC (the keeper: the "clean" option was chosen twice and reality vetoed it twice with evidence; the final answer is the initially-rejected one, chosen on measured grounds). a-18's top-5 was all changelog (287-chunk page, 8% of corpus). Option 1 = retrieval-time WHERE filter -> FALSIFIED #1: HNSW is approximate and POST-FILTERS its candidate set, so the filter returned 0 rows when a-18's nearest neighbours were all excluded. Option A = over-fetch + app-filter ("no GUC") -> FALSIFIED #2: HNSW caps candidates at ef_search (default 40), so LIMIT 50 returned only 40, all changelog. Both retrieval-time fixes (raise ef_search, or hnsw.iterative_scan) then required a per-query transaction + SET LOCAL, the exact complexity retrieval-time was chosen to avoid, and iterative_scan's relaxed_order returns approximately-sorted scores, which would corrupt the exact similarity ordering that P4.4 calibration and the receipt top-score depend on. Option C = INGESTION EXCLUSION chosen (Brandon reversed his own Option-1 ruling once every premise was individually falsified: retrieval-time is now the complex path; recoverability of release notes is value a how-to Q&A would never redeem; re-ingest is 2 cents). Re-ingest: 19 pages removed (changelog + 18 whats-new), chunks cascade-deleted; corpus 3501 -> 3132 (verified: 0 excluded chunks/documents, 0 null embeddings, coverage clean). THIRD wrinkle found + fixed at re-ingest: deleting 367 chunks left DEAD HNSW GRAPH ENTRIES (pgvector reclaims only on VACUUM), so a-18 returned 0 even from the clean unfiltered corpus; VACUUM fixed it (0 -> 5). Added a post-deletion VACUUM to the sync (scripts/ingest.ts, Tier 2) so future deletions keep the index healthy. Implementation: config.corpus.excludedPagePatterns (RAG-19); lib/rag/exclusion.ts (pure isExcludedPage, shared by discovery + coverage + eval); corpus.discoverPages filters at discovery; retrieveTopK reverted to a plain top-k + a PERMANENT under-return guard (Brandon req 4); getCoverage keeps a defensive filter; queries.countExcludedChunks + the eval's corpus-check assert 0 excluded chunks in the corpus (Brandon req 3); eval assertRetrievalInvariants (full-k + no-leak, DI-tested). 81 tests (6 new: isExcludedPage x3, assertRetrievalInvariants x3). Self-audit AC-01/02 met, EVAL-03 logged, config RAG-19-clean; tsc/lint/format/DS-14/SEC-01/build green. The under-return guard EARNED ITS KEEP: it caught both the ef_search under-fill and the dead-entry under-fill and refused to write a corrupt artifact each time.
docs · 2026-07-23 · RAG-01/RAG-23 amendment (Brandon-authorized, rule 1): corpus scope = llms.txt MINUS config.corpus.excludedPagePatterns (changelog, whats-new/*), applied at discovery; excluded pages are never ingested and the sync deletes any previously ingested, then VACUUMs for HNSW health. The §6 retrieval-time-exclusion language added earlier this session (never committed) was reverted: retrieval is a plain top-k. Foreclosure of "what changed in week X" logged as accepted for a how-to Q&A. Committed separately as `docs: amend RAG-01/RAG-23 for corpus-scope exclusion`.
P4.2 · 2026-07-23 · Retrieval eval runner. evals/lib/metrics.ts (pure hit@5/MRR/per-question, unit-tested), evals/lib/run.ts (runRetrievalEval imports the PRODUCTION retrieve() — EVAL-04, zero parallel impl; scores rounded to 3dp per the EVAL-05 amendment), evals/lib/artifact.ts (§8 run artifact embedding config snapshot + commit SHA, EVAL-17), evals/run-retrieval.ts entry + `npm run eval:retrieval`. Writes evals/runs/{run_id}.json + evals/latest.json (EVAL-16 render source). 7 unit tests (metric math incl multi-gold first-rank + boundary-excluded aggregate; EVAL-17 snapshot). vitest include extended for evals/**/*.test.ts. Self-audit EVAL-04 / EVAL-05(amended) / EVAL-17 green; tsc/lint/format/DS-14(evals json exempt per P4.1)/SEC-01/build clean; 75 tests total. FIRST REAL NUMBER (Brandon review, banked raw before any iteration): hit@5 = 0.6000, MRR = 0.4708 (n=20 answerable; 12 hits / 8 misses). Miss diagnosis: a-06/a-12/a-20 (+likely a-08) = gold-label precision (retrieval returns the right page's parent/sibling chunk, gold points one section deeper — measurement corrections for P4.3); a-07/a-10 (best-practices outranks the canonical page), a-11 (model-config didn't surface), a-18 (287-chunk changelog page pollution) = real retrieval characteristics. EVAL-05 determinism confirmed live: two runs → byte-identical hit@5/MRR + rankings; raw floats ~1e-5 (amendment logged below). Cost: 23 query embeddings, fractions of a cent (ENG-17 sanctioned spender). The gap to the 0.85 AC-01/02 floor is P4.3's iteration ladder — a first-try 0.85 would have meant too-easy a test set.
docs · 2026-07-23 · EVAL-05 amendment (Brandon-authorized per CLAUDE.md rule 1). P4.2's two-run live evidence: retrieval metrics (hit@5, MRR) and chunk rankings are byte-identical across runs, but raw cosine similarity scores wobble ~1e-5 at the 5th–6th decimal because OpenAI's embedding endpoint is not bit-deterministic for identical input. Amended EVAL-05 so the determinism contract is byte-identical metrics + rankings (a changed number = a changed system), with raw scores demoted to diagnostic detail stored at 3 decimals and documented as carrying that external-API noise. Rationale (Brandon): the guarantee EVAL-05 exists for lives in metrics/rankings; caching query embeddings (the bit-exact alternative, Option 2) would trade away the eval exercising the real production embed path for a fixture-maintenance obligation, and coarse 2dp rounding (Option 3) would blur the very decimals P4.4 calibration studies. Committed separately as `docs: amend EVAL-05 determinism contract`.
P4.1 · 2026-07-23 · Eval test set authored: evals/testset.json, 28 questions (20 answerable / 5 refusal / 3 boundary) per eval-harness §1. Answerable golds are deterministic chunk_ids (rag-design §3) drawn from and VERIFIED against the live corpus — dangling-gold check: all 30 golds present (EVAL-02). Coverage: 20 distinct top-level areas (how-it-works, quickstart, memory, context-window, hooks, subagents, skills, mcp, settings, permission-modes, model-config, agent-sdk, vs-code, github-actions, sandboxing, headless, costs, checkpointing, amazon-bedrock, plugins). Refusals are plausible off-corpus asks (fine-tuning, pricing negotiation, competitor comparison, offline/local-model, certification); boundaries are adjacent-but-uncovered (TTS output, exact max file size, scheduled auto-PR-review) with expected behavior recorded (may shift post-calibration). Questions phrased as real users ask, not chunk paraphrases (EVAL-01); in-file changelog (EVAL-03). Self-audit EVAL-01/02/03 green. BRANDON REVIEW: read all 28, approved with 2 rewords (a-07, a-20) to break mirrored verb phrasing with their gold headings — topic nouns are fair game, sentence-shape mirroring is not; logged in the file changelog. Coverage swaps (Agent SDK + plugins IN, statusline + prompt-caching OUT) approved; statusline/prompt-caching + enterprise/slack/worktrees/output-styles noted as append-mostly v1.1 candidates. Tier 2 (flagged, Brandon to veto): eval JSON fixtures exempted from DS-14 in scripts/check-em-dash.mjs — test questions simulate real user input (em dashes are natural per EVAL-01) and run artifacts carry model-generated answer text (already DS-14-exempt); eval CODE (evals/*.ts) is still checked. Reconciles DS-14 vs EVAL-01 where the frozen docs are silent on eval fixtures.
...
docs · 2026-07-23 · EVAL §4/§5 amendment (Brandon-authorized, rule 1). (1) Recorded judged baseline pass-rate = MEDIAN of the 3 measurement runs (EVAL-10; a single run is a noisy draw and near the AC-03 0.80 floor the min-vs-median gap is decision-relevant — the first P4.5 run drew 0.80 while the median was 0.90), and the artifact must store every run's pass rate + spread; per-question detail is retained from the median run. (2) A below-threshold CI judged run auto-re-runs to 3 and is scored on the median verdict (EVAL-11), same methodology as the baseline, so a single judge coin-flip near the floor can't false-red an unchanged system; genuine regressions still fail the median of 3 (implementation lands in P4.6). Committed separately as c90545c. KNOWN ITEM logged (Brandon): the judged runner does not instrument spend (est. ~$0.50/run from token math: ~28 Haiku generations + ~20 Sonnet judges ×3) — small, non-blocking, for a future pass.
P4.5 · 2026-07-23 · Judge, noise, median-of-3 baseline. evals/lib/judge.ts (Sonnet-4-6 judge @ temp 0, stronger tier than the Haiku generator EVAL-07; four binary checks grounded/citations_valid/complete/no_fabrication; strict JSON, parseJudgeVerdict throws on unparseable EVAL-08), judged.ts (runJudgedEval + pure answerPasses/declineOutcome/scoreDecline; two-tier refusal scoring = server gate OR decline-sentinel prefix EVAL-09), noise.ts (computeNoise: per-run pass rates, aggregate spread, per-check flip rate, M = max(spread, 1/answerable)), run-judged.ts (retrieval ×1 + judged ×3, records the MEDIAN run EVAL-10). 11 new pure unit tests, zero network (ENG-17); eval:judged script. BASELINE (median of 3, ~$1 sanctioned across 2 rounds): retrieval hit@5 0.85 / MRR 0.696; judged pass rate 0.90 (runs 0.90/0.90/0.85, spread 0.05, M 0.05); refusals 5/5, boundaries 3/3; AC-03 floor (≥0.80) met with margin. A-08 RE-ARGUMENT (EVAL-03, the P4.3 standing trigger fired by the judged eval — the generator declined the answerable a-08): golds corrected to claude-code/mcp-quickstart (rank 1, the add+verify walkthrough, never a gold) + #installing-mcp-servers (the recommended HTTP remote-add command); dropped the deprecated-SSE gold (doc: "SSE is deprecated, use HTTP") and the weak general-MCP intro; MRR 0.6583→0.6958, hit@5 unchanged; argued in the testset changelog; stays answerable. Two median-run failures, both within noise: a-08 (the generator still declines ~half the time — the gold fix corrected the RETRIEVAL label; the generation-side decline is the model noise the median absorbs), a-09 (no_fabrication false with no reason while grounded+cited true — a judge false-positive flip). Self-audit EVAL-07/08/09/10/12 + AC-03 green; 94 tests, tsc/lint/format/DS-14/SEC-01/build clean. Committed 456d9d4 (+ docs c90545c). PUSHED CI VERIFIED GREEN (Brandon-confirmed for 456d9d4): https://github.com/BrandonChurch93/Ask-the-Claude-Docs/actions/runs/30053685209.
P4.5-note · 2026-07-23 · SCHEDULED-SYNC RED DIAGNOSIS (Brandon Q: the 08:00 UTC cron Sync run #1 failed red 23s while the manual run #2 was green). ROOT CAUSE (evidence-backed; the Actions run log is not reachable from this session — gh unauthenticated, no token, MCP needs OAuth — so this is diagnosed from the database + git history, not the log): the FIRST scheduled Sync-corpus Action fired at 08:00 UTC BEFORE the GitHub repo secrets were configured, so lib/env.ts's boot-time zod validation threw at module load (missing ANTHROPIC_API_KEY/OPENAI_API_KEY/DATABASE_URL) BEFORE main() ever touched the DB — a seconds-fast red (23s total = checkout + setup-node + npm ci + an immediate throw, not a timeout or partial ingest). EVIDENCE: (1) sync_runs holds 6 rows, ALL status=success, ZERO failed, and ZERO rows in the entire 07:00–09:30 UTC cron window — the failure left no DB trace; the ingest failure-handler (ingest.ts catch → insertSyncRun then rethrow, RAG-22) records a failed row whenever the DB is reachable, and the DB was reachable all day (rows at 03:31/03:32/04:48/04:56/16:47/20:56 UTC), so any pipeline/fetch/DB-query error would have recorded one → the failure was PRE-DB, i.e. env-boot. (2) The P2.6 log already documents "Brandon added secrets + triggered a manual run … run 30026573762 SUCCESS" = sync_run #5 at 16:47 UTC — LATER than the 08:00 cron, so the secrets were not set when the scheduled run fired. (3) The green manual run uses the same code + same secret refs, and all 6 recorded runs (full-ingest + no-op + delta paths) succeeded, so the pipeline code is sound. CONCLUSION: not a code bug, not recurring, not a random transient — a one-time setup-ordering artifact (the first cron beat the secrets). TONIGHT'S 08:00 UTC RUN WILL BE GREEN: the secrets now exist (proven by the green manual run + sync_runs #5/#6). No fix required. Falsifiable prediction — run #1's log first error line is env.ts "Invalid environment configuration. Fix these variables …". OPTIONAL durable hardening → KNOWN ITEM FOR P7 (Brandon): the sync's external calls (llms.txt + page fetches) and the DB connect have no timeout/retry, so a genuine future 4 AM blip would red the nightly run and — if the DB itself is down — couldn't even self-record a failed row; add bounded retry + fetch timeout at P7. TO CONFIRM (Brandon-requested): verify tonight's 08:00 UTC scheduled Sync run concluded green and note it here — the falsifiable close on this diagnosis.
